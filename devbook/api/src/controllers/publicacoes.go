...
//Adicionar nova publicacao no DB
...
//Retorna as publicacoes que aparecem no feed do usuario
...
//Retorna 1 publicacao expecifica
...
func AtualizarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	repositorio := repositorios.NovoRepositorioPublicacoes(db)
	publicacaoSalvaNoDB, erro := repositorio.BuscarPorID(publicacaoID)
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
...
func DeletarPublicacao(w http.ResponseWriter, r *http.Request) {
	...
	if publicacaoSalvaNoDB.AutorID != usuarioID {
		respostas.Erro(w, http.StatusForbidden, errors.New("não é possível excluir uma publicacao que não seja a sua"))
		return
	}
...
//Subtrai uma curtira em uma publicacao
